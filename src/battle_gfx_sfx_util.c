...
bool8 BattleLoadAllHealthBoxesGfx(u8 state)
{
    ...
    if (state != 0)
    {
        ...
        else if (!IsDoubleBattle())
        {
            ...
            {
                LoadCompressedSpriteSheet(&sSpriteSheets_HealthBar[gBattlerPositions[B_BATTLER_PLAYER_LEFT]]);
            }
            else if (state == 5)
            {
                LoadCompressedSpriteSheet(&sSpriteSheets_HealthBar[gBattlerPositions[B_BATTLER_OPPONENT_LEFT]]);
            }
            else
            {
                retVal = TRUE;
        ...
        else
        {
            if (state == 2)
                ...
                LoadCompressedSpriteSheet(&sSpriteSheets_HealthBar[gBattlerPositions[B_BATTLER_PLAYER_RIGHT]]);
            else if (state == 9)
                LoadCompressedSpriteSheet(&sSpriteSheets_HealthBar[gBattlerPositions[B_BATTLER_OPPONENT_RIGHT]]);
            else
                retVal = TRUE;
        }
    }
