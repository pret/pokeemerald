        ...
        do
        {
            ...
            case B_POSITION_PLAYER_RIGHT:
                if (gActiveBattler != gMultiUsePlayerCursor)
                    i++;
                else if (gBattleMoves[GetMonData_2(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_MOVE1 + gMoveSelectionCursor[gActiveBattler])].target & MOVE_TARGET_USER_OR_SELECTED)
                    i++;
                break;
    ...
    if (gMain.newKeys & (A_BUTTON | SELECT_BUTTON))
    {
        ...
        if (gMoveSelectionCursor[gActiveBattler] != gMultiUsePlayerCursor)
        {
            ...
            if (!(gBattleMons[gActiveBattler].status2 & STATUS2_TRANSFORMED))
            {
                for (i = 0; i < MAX_MON_MOVES; i++)
                {
                    moveStruct.moves[i] = GetMonData_2(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_MOVE1 + i);
                    moveStruct.currentPp[i] = GetMonData_2(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_PP1 + i);
                }

                totalPPBonuses = GetMonData_2(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_PP_BONUSES);
                for (i = 0; i < MAX_MON_MOVES; i++)
...
static void Task_GiveExpToMon(u8 taskId)
{
    ...
    if (IsDoubleBattle() == TRUE || monId != gBattlerPartyIndexes[battlerId]) // Give exp without moving the expbar.
    {
        ...
        u8 level = GetMonData_2(mon, MON_DATA_LEVEL);
        u32 currExp = GetMonData_2(mon, MON_DATA_EXP);
...
static void Task_PrepareToGiveExpWithExpBar(u8 taskId)
{
    ...
    u8 level = GetMonData_2(mon, MON_DATA_LEVEL);
    u16 species = GetMonData_2(mon, MON_DATA_SPECIES);
    u32 exp = GetMonData_2(mon, MON_DATA_EXP);
...
static void sub_8059400(u8 taskId)
{
    ...
    else
    {
        ...
        if (newExpPoints == -1) // The bar has been filled with given exp points.
        {
            ...
            level = GetMonData_2(&gPlayerParty[monId], MON_DATA_LEVEL);
            currExp = GetMonData_2(&gPlayerParty[monId], MON_DATA_EXP);
            species = GetMonData_2(&gPlayerParty[monId], MON_DATA_SPECIES);
...
static void Task_UpdateLvlInHealthbox(u8 taskId)
{
    ...
    if (!gBattleSpritesDataPtr->healthBoxesData[battlerId].specialAnimActive)
    {
        ...
        GetMonData_2(&gPlayerParty[monIndex], MON_DATA_LEVEL);  // Unused return value.
...
static void PlayerHandleExpUpdate(void)
{
    ...
    if (GetMonData_2(&gPlayerParty[monId], MON_DATA_LEVEL) >= MAX_LEVEL)
    {
        PlayerBufferExecCompleted();
    }
    else
    {
        ...
        LoadBattleBarGfx(1);
        GetMonData_2(&gPlayerParty[monId], MON_DATA_SPECIES);  // Unused return value.
...
static void PlayerHandleStatusXor(void)
{
    u8 val = GetMonData_2(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_STATUS) ^ gBattleBufferA[gActiveBattler][1];
