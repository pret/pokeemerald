...
void PressurePPLoseOnUsingPerishSong(u8 attacker)
{
    ...
    {
        gActiveBattler = attacker;
        BtlController_EmitSetMonData(BATTLELINKCOMMTYPE_ENGINE_TO_CONTROLLER, REQUEST_PPMOVE1_BATTLE + perishSongPos, 0, 1, &gBattleMons[gActiveBattler].pp[perishSongPos]);
        MarkBattlerForControllerExec(gActiveBattler);
    }
}
...
void PrepareStringBattle(u16 stringId, u8 battler)
{
    ...
    BtlController_EmitPrintString(BATTLELINKCOMMTYPE_ENGINE_TO_CONTROLLER, stringId);
    MarkBattlerForControllerExec(gActiveBattler);
}
            ...
            if (gBattleMons[battler].hp != 0)
            {
                ...
                case ABILITY_SHED_SKIN:
                    if ((gBattleMons[battler].status1 & STATUS1_ANY) && (Random() % 3) == 0)
                    {
                        ...
                        BtlController_EmitSetMonData(BATTLELINKCOMMTYPE_ENGINE_TO_CONTROLLER, REQUEST_STATUS_BATTLE, 0, 4, &gBattleMons[battler].status1);
                        MarkBattlerForControllerExec(gActiveBattler);
                        effect++;
                    }
                    break;
        ...
        if (gBattleMons[battlerId].hp)
        {
            ...
            case HOLD_EFFECT_RESTORE_PP:
                if (!moveTurn)
                {
                    ...
                    if (i != MAX_MON_MOVES)
                    {
                        ...
                        BtlController_EmitSetMonData(BATTLELINKCOMMTYPE_ENGINE_TO_CONTROLLER, i + REQUEST_PPMOVE1_BATTLE, 0, 1, &changedPP);
                        MarkBattlerForControllerExec(gActiveBattler);
            ...
            if (effect != 0)
            {
                ...
                case ITEM_STATUS_CHANGE:
                    BtlController_EmitSetMonData(BATTLELINKCOMMTYPE_ENGINE_TO_CONTROLLER, REQUEST_STATUS_BATTLE, 0, 4, &gBattleMons[battlerId].status1);
                    MarkBattlerForControllerExec(gActiveBattler);
                    break;
                case ITEM_PP_CHANGE:
