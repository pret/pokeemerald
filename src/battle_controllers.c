        ...
        for (i = 0; i < gBattlersCount; i++)
        {
            for (j = 0; j < PARTY_SIZE; j++)
            {
                if (i < 2)
                {
                    ...
                    {
                        if (GetMonData_2(&gPlayerParty[j], MON_DATA_HP) != 0
                         && GetMonData_2(&gPlayerParty[j], MON_DATA_SPECIES2) != SPECIES_NONE
                         && GetMonData_2(&gPlayerParty[j], MON_DATA_SPECIES2) != SPECIES_EGG
                         && GetMonData_2(&gPlayerParty[j], MON_DATA_IS_EGG) == 0)
                        {
                    ...
                    {
                        if (GetMonData_2(&gEnemyParty[j], MON_DATA_HP) != 0
                         && GetMonData_2(&gEnemyParty[j], MON_DATA_SPECIES2) != SPECIES_NONE
                         && GetMonData_2(&gEnemyParty[j], MON_DATA_SPECIES2) != SPECIES_EGG
                         && GetMonData_2(&gEnemyParty[j], MON_DATA_IS_EGG) == 0)
                        {
                            gBattlerPartyIndexes[i] = j;
                            break;
                        }
                ...
                else
                {
                    ...
                    {
                        if (GetMonData_2(&gPlayerParty[j], MON_DATA_HP) != 0
                         && GetMonData_2(&gPlayerParty[j], MON_DATA_SPECIES) != SPECIES_NONE  // Probably a typo by Game Freak. The rest use SPECIES2.
                         && GetMonData_2(&gPlayerParty[j], MON_DATA_SPECIES2) != SPECIES_EGG
                         && GetMonData_2(&gPlayerParty[j], MON_DATA_IS_EGG) == 0
