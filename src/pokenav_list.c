...
struct PokenavList
{
    struct PokenavListView view;
    struct PokenavListWindowState windowState;
    s32 eraseIndex;
    u32 loopedTaskId;
};

static void InitPokenavListBg(struct PokenavListView *);
static bool32 CopyPokenavListMenuTemplate(struct PokenavListView *, const struct BgTemplate *, const struct PokenavListTemplate *, u32);
...
static void ToggleListArrows(struct PokenavListView *, bool32);
static void DestroyListArrows(struct PokenavListView *);
static void CreateListArrowSprites(const struct PokenavListWindowState *, struct PokenavListView *);
static void LoadListArrowGfx(void);
static void PrintMatchCallFlavorText(struct PokenavListWindowState *, struct PokenavListView *, u32);
static void PrintMatchCallFieldNames(struct PokenavListView *, u32);
static void PrintMatchCallListTrainerName(struct PokenavListWindowState *, struct PokenavListView *);
static void PrintCheckPageTrainerName(struct PokenavListWindowState *, struct PokenavListView *);
static void EraseListEntry(struct PokenavListMenuWindow *, s32, s32);
static void CreateMoveListWindowTask(s32, struct PokenavListView *);
static void PrintListItems(void *, u32, u32, u32, u32, struct PokenavListView *);
static void InitListItems(const struct PokenavListWindowState *, struct PokenavListView *);
...
bool32 CreatePokenavList(const struct BgTemplate *bgTemplate, struct PokenavListTemplate *listTemplate, u32 tileOffset)
{
    ...
    if (!CopyPokenavListMenuTemplate(&list->view, bgTemplate, listTemplate, tileOffset))
        return FALSE;
...
void DestroyPokenavList(void)
{
    ...
    DestroyListArrows(&list->view);
    RemoveWindow(list->view.listWindow.windowId);
...
static u32 LoopedTask_CreatePokenavList(s32 state)
{
    ...
    {
    case 0:
        InitPokenavListBg(&list->view);
        return LT_INC_AND_PAUSE;
    case 1:
        InitPokenavListWindow(&list->view.listWindow);
        return LT_INC_AND_PAUSE;
    case 2:
        InitListItems(&list->windowState, &list->view);
        return LT_INC_AND_PAUSE;
    case 3:
    ...
    case 4:
        CreateListArrowSprites(&list->windowState, &list->view);
        return LT_FINISH;
    default:
        return LT_FINISH;
    }
...
static void InitPokenavListBg(struct PokenavListView *view)
{
    u16 tileNum = (view->listWindow.fillValue << 12) | view->listWindow.tileOffset;
    BgDmaFill(view->listWindow.bg, PIXEL_FILL(1), view->listWindow.tileOffset, 1);
    BgDmaFill(view->listWindow.bg, PIXEL_FILL(4), view->listWindow.tileOffset + 1, 1);
    SetBgTilemapBuffer(view->listWindow.bg, view->tilemapBuffer);
    FillBgTilemapBufferRect_Palette0(view->listWindow.bg, tileNum, 0, 0, 32, 32);
    ChangeBgY(view->listWindow.bg, 0, BG_COORD_SET);
    ChangeBgX(view->listWindow.bg, 0, BG_COORD_SET);
    ChangeBgY(view->listWindow.bg, view->listWindow.y << 11, BG_COORD_SUB);
    CopyBgTilemapBufferToVram(view->listWindow.bg);
}
...
static void InitListItems(const struct PokenavListWindowState *windowState, struct PokenavListView *viewPtr)
{
    ...
    PrintListItems(windowState->listPtr, windowState->windowTopIndex, numToPrint, windowState->listItemSize, 0, viewPtr);
}

static void PrintListItems(void *listPtr, u32 topIndex, u32 numItems, u32 itemSize, u32 printStart, struct PokenavListView *list)
{
    if (numItems == 0)
        return;
...
{
    u32 row;
    struct PokenavListView *listSub = GetSubstructPtr(POKENAV_SUBSTRUCT_LIST);
...
static void MoveListWindow(s32 delta, bool32 printItems)
{
    ...
    if (delta < 0)
    {
        ...
        if (printItems)
            PrintListItems(windowState->listPtr, windowState->windowTopIndex + delta, delta * -1, windowState->listItemSize, delta, &list->view);
    }
    else if (printItems)
    {
        ...
        PrintListItems(windowState->listPtr, index, delta, windowState->listItemSize, windowState->entriesOnscreen, &list->view);
    }

    CreateMoveListWindowTask(delta, &list->view);
    windowState->windowTopIndex += delta;
}

static void CreateMoveListWindowTask(s32 delta, struct PokenavListView *list)
{
...
static u32 LoopedTask_MoveListWindow(s32 state)
{
    ...
    bool32 finished;
    struct PokenavList *list = GetSubstructPtr(POKENAV_SUBSTRUCT_LIST);
    struct PokenavListView *viewPtr = &list->view;
    ...
    case 1:
        finished = FALSE;
        oldY = GetBgY(viewPtr->listWindow.bg);
        newY = ChangeBgY(viewPtr->listWindow.bg, 0x1000, viewPtr->bgMoveType);
        if (viewPtr->bgMoveType == BG_COORD_SUB)
        {
            if ((oldY > viewPtr->endBgY || oldY <= viewPtr->startBgY) && newY <= viewPtr->endBgY)
                finished = TRUE;
        }
        else // BG_COORD_ADD
        {
        ...
        if (finished)
        {
            viewPtr->listWindow.unkA = (viewPtr->listWindow.unkA + viewPtr->moveDelta) & 0xF;
            ChangeBgY(viewPtr->listWindow.bg, viewPtr->endBgY, BG_COORD_SET);
            return LT_FINISH;
        }
        return LT_PAUSE;
    }
...
{
    struct PokenavList *list = GetSubstructPtr(POKENAV_SUBSTRUCT_LIST);
    return IsLoopedTaskActive(list->view.loopedTaskId);
}
...
void PokenavList_DrawCurrentItemIcon(void)
{
    struct PokenavList *list = GetSubstructPtr(POKENAV_SUBSTRUCT_LIST);
    struct PokenavListWindowState *windowState = &list->windowState;
    list->view.iconDrawFunc(list->view.listWindow.windowId, windowState->windowTopIndex + windowState->selectedIndexOffset, (list->view.listWindow.unkA + windowState->selectedIndexOffset) & 0xF);
    CopyWindowToVram(list->view.listWindow.windowId, COPYWIN_MAP);
}
...
static u32 LoopedTask_EraseListForCheckPage(s32 state)
{
    ...
    {
    case 0:
        ToggleListArrows(&list->view, TRUE);
        // fall-through
    case 1:
        if (list->eraseIndex != list->windowState.selectedIndexOffset)
            EraseListEntry(&list->view.listWindow, list->eraseIndex, 1);
    ...
    case 2:
        if (!IsDma3ManagerBusyWithBgCopy())
        {
            ...
            if (list->windowState.selectedIndexOffset != 0)
                EraseListEntry(&list->view.listWindow, list->eraseIndex, list->windowState.selectedIndexOffset);
...
static u32 LoopedTask_PrintCheckPageInfo(s32 state)
{
    ...
    {
    case 0:
        PrintCheckPageTrainerName(&list->windowState, &list->view);
        break;
    case 1:
        PrintMatchCallFieldNames(&list->view, 0);
        break;
    case 2:
        PrintMatchCallFlavorText(&list->windowState, &list->view, CHECK_PAGE_STRATEGY);
        break;
    case 3:
        PrintMatchCallFieldNames(&list->view, 1);
        break;
    case 4:
        PrintMatchCallFlavorText(&list->windowState, &list->view, CHECK_PAGE_POKEMON);
        break;
    case 5:
        PrintMatchCallFieldNames(&list->view, 2);
        break;
    case 6:
        PrintMatchCallFlavorText(&list->windowState, &list->view, CHECK_PAGE_INTRO_1);
        break;
    case 7:
        PrintMatchCallFlavorText(&list->windowState, &list->view, CHECK_PAGE_INTRO_2);
        break;
    default:
        return LT_FINISH;
    }
...
static u32 LoopedTask_ReshowListFromCheckPage(s32 state)
{
    struct PokenavList *list;
    struct PokenavListWindowState *windowState;
    struct PokenavListView *viewPtr;
    s32 r5, *ptr;
    ...
    case 0:
        ...
        PrintMatchCallListTrainerName(windowState, viewPtr);
        return LT_INC_AND_PAUSE;
    case 1:
        ptr = &list->eraseIndex;
        if (++(*ptr) < list->windowState.entriesOnscreen)
        {
            EraseListEntry(&viewPtr->listWindow, *ptr, 1);
            return LT_PAUSE;
        }
        ...
        *ptr = 0;
        if (windowState->listLength <= windowState->entriesOnscreen)
        {
            if (windowState->windowTopIndex != 0)
            {
                ...
                r5 = -r4;
                EraseListEntry(&viewPtr->listWindow, r5, r4);
    ...
    case 3:
        ...
        return LT_PAUSE;
    case 4:
        PrintListItems(windowState->listPtr, windowState->windowTopIndex + list->eraseIndex, 1, windowState->listItemSize, list->eraseIndex, &list->view);
        return LT_INC_AND_PAUSE;
    case 5:
        if (IsPrintListItemsTaskActive())
            return LT_PAUSE;
        ...
        return LT_SET_STATE(4);
    case 6:
        ToggleListArrows(viewPtr, FALSE);
        return LT_FINISH;
    }
...
// Pointless
static void SetListMarginTile(const struct PokenavListMenuWindow *listWindow, bool32 draw)
{
    u16 var;
...
static void PrintCheckPageTrainerName(struct PokenavListWindowState *state, struct PokenavListView *list)
{
...
static void PrintMatchCallListTrainerName(struct PokenavListWindowState *state, struct PokenavListView *list)
{
    list->bufferItemFunc(state->listPtr + state->listItemSize * state->windowTopIndex, list->itemTextBuffer);
...
static void PrintMatchCallFieldNames(struct PokenavListView *list, u32 fieldId)
{
    const u8 *fieldNames[] = {
...
static void PrintMatchCallFlavorText(struct PokenavListWindowState *windowState, struct PokenavListView *list, u32 checkPageEntry)
{
...
static void CreateListArrowSprites(const struct PokenavListWindowState *windowState, struct PokenavListView *list)
{
    u32 spriteId;
    s16 x;
...
static void DestroyListArrows(struct PokenavListView *list)
{
...
static void ToggleListArrows(struct PokenavListView *list, bool32 invisible)
{
    if (invisible)
    {
...
{
    struct PokenavList *list = GetSubstructPtr(POKENAV_SUBSTRUCT_LIST);
    list->view.upArrow->sInvisible = invisible;
    list->view.downArrow->sInvisible = invisible;
}
...
static bool32 CopyPokenavListMenuTemplate(struct PokenavListView *view, const struct BgTemplate *bgTemplate, const struct PokenavListTemplate *template, u32 tileOffset)
{
    struct WindowTemplate window;

    view->listWindow.bg = bgTemplate->bg;
    view->listWindow.tileOffset = tileOffset;
    view->bufferItemFunc = template->bufferItemFunc;
    view->iconDrawFunc = template->iconDrawFunc;
    view->listWindow.fillValue = template->fillValue;
    view->listWindow.x = template->item_X;
    view->listWindow.y = template->listTop;
    view->listWindow.width = template->windowWidth;
    view->listWindow.fontId = template->fontId;
    ...
    view->listWindow.windowId = AddWindow(&window);
    if (view->listWindow.windowId == WINDOW_NONE)
        return FALSE;

    view->listWindow.unkA = 0;
