...
static bool32 AnyMonHasRibbon(void)
{
    ...
    {
        if (GetMonData_2(&gPlayerParty[i],  MON_DATA_SANITY_HAS_SPECIES)
            && !GetMonData_2(&gPlayerParty[i], MON_DATA_SANITY_IS_EGG)
            && GetMonData_2(&gPlayerParty[i], MON_DATA_RIBBON_COUNT) != 0)
        {
            return TRUE;
        }
    }
