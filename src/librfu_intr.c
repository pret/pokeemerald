...
static void sio32intr_clock_master(void)
{
    ...
    if (gSTWIStatus->state == 0) // master send req
    {
        if (regSIODATA32 == 0x80000000)
        {
            ...
            {
                REG_SIODATA32 = (&gSTWIStatus->txPacket->rfuPacket32.command)[gSTWIStatus->reqNext];
                gSTWIStatus->reqNext++;
            }
            else
            {
    ...
    else if (gSTWIStatus->state == 1) // master wait ack
    {
        if ((regSIODATA32 & 0xFFFF0000) == 0x99660000)
        {
            ...
            (&gSTWIStatus->rxPacket->rfuPacket32.command)[gSTWIStatus->ackNext] = regSIODATA32;
            gSTWIStatus->ackNext++;
            gSTWIStatus->ackActiveCommand = (u8)regSIODATA32;
            ackLen = regSIODATA32 >> 8;
            gSTWIStatus->ackLength = ackLen;
            // ackLen is truncated to u8 when stored in gSTWIStatus->ackLength.
            ackLen = gSTWIStatus->ackLength;
            if (ackLen >= gSTWIStatus->ackNext)
            {
...
static void sio32intr_clock_slave(void)
{
    ...
    {
        *(u32 *)&gSTWIStatus->rxPacket->rfuPacket32.command = regSIODATA32;
        ...
        if (reqLen == (r0 >> 16))
        {
            ...
            gSTWIStatus->reqLength = reqLen;
            gSTWIStatus->reqActiveCommand = (u8)regSIODATA32;
            if (gSTWIStatus->reqLength == 0)
            {
                ...
                )
                {
                    ...
                    *(u32 *)&gSTWIStatus->txPacket->rfuPacket32.command = 0x99660000 + gSTWIStatus->ackActiveCommand;
                    gSTWIStatus->ackLength = 0;
                }
                else
                {
                    *(u32 *)&gSTWIStatus->txPacket->rfuPacket32.command = 0x996601EE;
                    if (gSTWIStatus->reqActiveCommand >= 0x10 && gSTWIStatus->reqActiveCommand <= 0x3D)
                    {
                        gSTWIStatus->txPacket->rfuPacket32.data[0] = 1;
                    }
                    else
                    {
                        gSTWIStatus->txPacket->rfuPacket32.data[0] = 2;
                    }
    ...
    {
        (&gSTWIStatus->rxPacket->rfuPacket32.command)[gSTWIStatus->reqNext] = regSIODATA32;
        gSTWIStatus->reqNext++;
    ...
    else if (gSTWIStatus->state == 7) // slave send ack
    {
        if (regSIODATA32 == 0x80000000)
        {
            ...
            {
                REG_SIODATA32 = (&gSTWIStatus->txPacket->rfuPacket32.command)[gSTWIStatus->ackNext];
                gSTWIStatus->ackNext++;
            }
        }
        else
