...
static void InitCanRelaseMonVars(void)
{
    ...
    sPSSData->field_2174 = GetMonDataExtended(&sPSSData->field_2108, MON_DATA_KNOWN_MOVES, (u8*)sPSSData->field_2176);
...
static s8 RunCanReleaseMon(void)
{
    ...
    case 0:
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            {
                knownMoves = GetMonDataExtended(gPlayerParty + i, MON_DATA_KNOWN_MOVES, (u8*)sPSSData->field_2176);
...
static void SetCursorMonData(void *pokemon, u8 mode)
{
    ...
    if (mode == MODE_PARTY)
    {
        ...
        if (sPSSData->cursorMonSpecies != SPECIES_NONE)
        {
            ...
            GetMonDataExtended(mon, MON_DATA_NICKNAME, sPSSData->cursorMonNick);
    ...
    else if (mode == MODE_BOX)
    {
        ...
        if (sPSSData->cursorMonSpecies != SPECIES_NONE)
        {
            ...
            GetBoxMonDataExtended(boxMon, MON_DATA_NICKNAME, sPSSData->cursorMonNick);
            StringGetEnd10(sPSSData->cursorMonNick);
...
{
    if (boxId < TOTAL_BOXES_COUNT && boxPosition < IN_BOX_COUNT)
        GetBoxMonDataExtended(&gPokemonStoragePtr->boxes[boxId][boxPosition], MON_DATA_NICKNAME, dst);
    else
        *dst = EOS;
}
...
{
    if (boxId < TOTAL_BOXES_COUNT && boxPosition < IN_BOX_COUNT)
        return GetBoxMonDataExtended(&gPokemonStoragePtr->boxes[boxId][boxPosition], request, dst);
    else
        return 0;
}
...
bool32 AnyStorageMonWithMove(u16 moveId)
{
    ...
    for (i = 0; i < TOTAL_BOXES_COUNT; i++)
    {
        for (j = 0; j < IN_BOX_COUNT; j++)
        {
            if (GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], MON_DATA_SANITY_HAS_SPECIES)
                && !GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], MON_DATA_SANITY_IS_EGG)
                && GetBoxMonDataExtended(&gPokemonStoragePtr->boxes[i][j], MON_DATA_KNOWN_MOVES, (u8*)moves))
                return TRUE;
        }
    }
